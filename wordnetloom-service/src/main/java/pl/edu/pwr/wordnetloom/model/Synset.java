...
public class Synset implements Serializable {
    ...
    public Synset(Integer split, List<SenseToSynset> senseToSynset, List<SynsetAttribute> synsetAttributes) {
        this.split = split;

        for (SenseToSynset sts : senseToSynset) {
            this.senseToSynset.add(new SenseToSynset(this, sts.getSense(), sts.getSenseIndex()));
        }
        for (SynsetAttribute sa : synsetAttributes) {
            this.synsetAttributes.add(new SynsetAttribute(sa, this));
        }
    }
    ...
    public boolean isEqual(Object second) {
        if (second instanceof Synset) {
            ...
            return id.longValue() == synset.id.longValue();
        }
        return false;
    }
