...
public class UnitsListFrame extends AbstractListFrame<Sense, PartOfSpeech> {
		...
		if (this.filterObject != null)
			for (Sense elem : this.selectedElements) {
				if (LexicalDA.checkIfInAnySynset(elem)) {
					DialogBox
							.showError(String.format(Messages.INFO_UNIT_ALREADY_ASSIGNED_TO_SYNSET, (elem).getLemma()));
					return false;
				}
			}
		return true;
	 ...
	 *            - położenie Y
	 * @param multiSelect
	 *            - pozwala zaznaczyc wiecej niz jeden obiekt na liscie
	 * @param filterObject
	 *            - obiekt filtrujący
	 * @param unitWasCreated
	 *            - parametr wyjsciowy, przechowuje informacje o tym czy nowa
	 *            jednostka zostala stworzona
	 * @return zaznaczone elemty albo null gdy anulowano
	 */
	static public Collection<Sense> showModal(Workbench workbench, int x, int y, boolean multiSelect,
			PartOfSpeech filterObject, ValueContainer<Boolean> unitWasCreated) {
