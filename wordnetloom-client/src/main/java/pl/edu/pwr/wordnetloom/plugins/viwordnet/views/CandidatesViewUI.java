...
import pl.edu.pwr.wordnetloom.plugins.relations.RelationsIM;
...
import pl.edu.pwr.wordnetloom.plugins.candidates.tasks.RebuildGraphsTask;
...
public class CandidatesViewUI extends AbstractViewUI implements ListSelectionListener, ActionListener, ChangeListener {

	private HashMap<Pair<Integer, PartOfSpeech>, RebuildGraphsTask> tasks = new HashMap<Pair<Integer, PartOfSpeech>, RebuildGraphsTask>();

	private ListOrderedSet<ListElement> list_elems = new ListOrderedSet<ListElement>();

	private SimpleListenersContainer candidateChanged_ = new SimpleListenersContainer();
	...
	private DefaultListModel<ListElement> candsModel = null;

	private JList<ListElement> candsList;
	private JTextField filterEdit;
	private JSpinner pckgSpinner;
	private ComboBoxPlain<PartOfSpeech> posCombo;
	...
	protected void initialize(JPanel content) {
		...
		((JSpinner.DefaultEditor) pckgSpinner.getEditor()).getTextField().addKeyListener(new KeyAdapter() {
	...
	private void loadSpinnerModel() {
		...
		currentMaxPkg = RemoteUtils.extGraphRemote
				.GetMaxPackageNo((PartOfSpeech) posCombo.getItemAt(posCombo.getSelectedIndex()));
		if (currentMaxPkg == -1)
			currentMaxPkg = 0;
		infoPackages.setText("(" + currentMaxPkg + ")");

		Collection<Integer> list_of_packages = RemoteUtils.extGraphRemote
				.GetPackages((PartOfSpeech) posCombo.getItemAt(posCombo.getSelectedIndex()));
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (!e.getValueIsAdjusting() && !candsList.isSelectionEmpty()) {
			ListElement elem = (ListElement) candsList.getSelectedValue();
			candidateChanged_.notifyAllListeners(new Pair<String, PartOfSpeech>(elem.getWord(), elem.getPos()),
					elem.getPkg());
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == filterEdit || e.getSource() == buttonSearch) {
			filterList();
			infoLabel.setText(String.format(Labels.NUMBER_OF_ELEMENTS, Integer.toString(candsModel.size())));
		} else if (e.getSource() == posCombo) {
			loadSpinnerModel();
		} else if (e.getSource() == buttonShow || e.getSource() == pckgSpinner) {
			if (currentMaxPkg != 0) {
				list_elems.clear();
				loadListData();
		...
		} else if (e.getSource() == buttonRecalc && currentMaxPkg != 0) {
			Integer packageNo = ((Integer) packageNoModel.getValue()).intValue();
			PartOfSpeech pos = (PartOfSpeech) posCombo.getItemAt(posCombo.getSelectedIndex());

			RebuildGraphsTask t = tasks.get(new Pair<Integer, PartOfSpeech>(new Integer(packageNo), pos));
			...
			if (DialogBox.showConfirmDialog(Messages.QUESTION_DO_YOU_WANT_TO_RECALCULATE_GRAPHS,
					Labels.GRAPHS_CALCULATING, JOptionPane.INFORMATION_MESSAGE,
					JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				RebuildGraphsTask t1 = new RebuildGraphsTask(pos, packageNo, this);
				tasks.put(new Pair<Integer, PartOfSpeech>(new Integer(packageNo), pos), t1);
			}
		}
	}
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (e.getSource() instanceof RebuildGraphsTask) {
			synchronized (tasks) {
				...
				Pair<Integer, PartOfSpeech> key = new Pair<Integer, PartOfSpeech>(new Integer(t.getPackageNo()),
						t.getPos());
