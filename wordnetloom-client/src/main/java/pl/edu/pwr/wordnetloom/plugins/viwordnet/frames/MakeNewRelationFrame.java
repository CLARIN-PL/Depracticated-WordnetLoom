...
public class MakeNewRelationFrame extends RelationTypeFrame {
			...
			for (RelationType type : mainRelations) {
				if (index-- == 0) {
					...
					Collection<RelationType> readRelations = LexicalDA
							.getChildren(type);
					for (RelationType relType : readRelations) {
						if (fixedRelationType == null || // gdy nie zdefiniowana
															// relacji
	...
	/**
	 * @param workbench
	 *            <code>Workbench</code> to get JFrame
	 * @param from
	 *            <code>ViwnNode</code> parent for relation
	 * @param to
	 *            <code>ViwnNode</code> child for relation
		...
		if (framew.chosenType != null) {
			...
			// make relation
				...
				if (framew.chosenType.isAutoReverse()
					...
					if (framew.chosenType.isAutoReverse()
							|| hasReversRelation &&( 
					DialogBox.showYesNo(String.format(Messages.QUESTION_CREATE_CONNECTION_FOR_REVERSE_RELATION+ test,
