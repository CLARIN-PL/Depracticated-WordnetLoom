...
package pl.edu.pwr.wordnetloom.plugins.relations.da;
...
import pl.edu.pwr.wordnetloom.systems.progress.ProgressFrame;
...
public class RelationsDA {
	 ...
	 * @param relationType - typ relacji jakie muszą byc zdefiniowane dla wynikowych synsetow
	...
	/**
	 * odczytane listy synsetow spelnijacych filtr, dodane filtrowanie po części mowy
	 * @param filterText - filtr nazw
	 * @param status - status do filtrowania (dochodzi do tego 0- wszystko)
	 * @param domainStr - domena do filtrowania
	 * @param relationType - typ relacji jakie muszą byc zdefiniowane dla wynikowych synsetow
...
//	}
	 ...
	 * odczytanie jednostek synsetu
	 * @param synset - synset
	 * @param ignoredUnits - lista jednostke ktore maja zostac zignorowane przy oczycie
	 * @param lexicons 
	 * @return jednostki synsetu
	 */
	...
	/**
	 * odczytanie wszystkich typow relacji dla danego synsetu ktorymi jest zwiazany
	 * z innymi
	 * @param synset - synset
...
//	}
	 ...
	 * zbudowanie drzewa relacji synsetow
	 * 
	 * Funkcja zmodyfikowana w celu redukcji pobieranych rekordów synset description.
	 * Pobierane są tylko potrzebne ID, zamiast całej listy.
	 * 
	 * @param parent
	 * @param synset
	 * @param relationType
...
//		RemoteUtils.lexicalUnitRemote.dbDelete(src);
	...
	public static void mergeSynsets(Synset src, Synset dst, List<Long> lexicons) {
		...
		if (old_comment.isEmpty())
//			Common.addSynsetAttribute(dst, Synset.COMMENT, src_comment);
			RemoteUtils.dynamicAttributesRemote.saveOrUpdateSynsetAttribute(dst, Synset.COMMENT, src_comment);
		else if (!src_comment.isEmpty())
//			Common.addSynsetAttribute(dst, Synset.COMMENT, old_comment + "; " + src_comment);
