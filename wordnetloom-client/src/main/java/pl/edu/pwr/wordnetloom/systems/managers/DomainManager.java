...
public class DomainManager {
	...
	/**
	 * The most important method, it fetches data from database and puts it in the cashe to later use.
	 * Another task for this method is to update managers' keys.
	 * @param languageCode
	 */
	private void fetchDataFromDB(String languageCode) {
		...
		synchronized (languageCode) {
			 ...
			 * fixme
			 */
			DomainServiceRemote service = RMIUtils.lookupForService(DomainServiceRemote.class);
			List<Domain> list = service.getAllDomains(LexiconManager.getInstance().getLexicons());
	...
	/**
	 * Method to refresh current manager data with current language code from database.
	 */
	public void refresh(){
	 ...
	 * @return default {@link DomainManager} instance if exists, throws exception otherwise.
	 */
	 ...
	 * @return specific {@link DomainManager}, this method won't throw exception if no manager is found,
	...
	/**
	 * Simple init, without return. Use it, preferably in other thread, if you do not want to hinder and slow down
	 * (suspend even) Swing's Event Dispatcher Thread. Don't worry about concurrency, this whole manager is as thread
	 * safe as it could possibly get.
