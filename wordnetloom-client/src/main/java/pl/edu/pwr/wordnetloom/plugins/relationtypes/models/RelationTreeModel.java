...
package pl.edu.pwr.wordnetloom.plugins.relationtypes.models;
...
//import pl.wroc.pwr.ci.plwordnet.utils.PairUtils;
...
public class RelationTreeModel implements TreeModel {
	...
	public Object getChild(Object obj, int index) {
		...
		Collection<RelationType> children = RemoteUtils.relationTypeRemote.dbGetChildren(relType,
	...
	public boolean isLeaf(Object obj) {
		...
		RelationType relType = (RelationType) obj;
		return RemoteUtils.relationTypeRemote.dbGetChildren(relType, LexiconManager.getInstance().getLexicons())
				.size() == 0;
	}
	...
	public int getIndexOfChild(Object arg0, Object arg1) {
		...
		if (arg0 == root) {
			...
			for (RelationType c : RemoteUtils.relationTypeRemote.dbGetChildren(parent,
					LexiconManager.getInstance().getLexicons())) {
				if (c.equals(child))
					return i;
				i++;
	...
	private void moveRel_(TreePath path, int offset) {
		...
		if (parent == null) {
			...
			RelationType[] children = new RelationType[RemoteUtils.relationTypeRemote
					.dbGetChildren(parent, LexiconManager.getInstance().getLexicons()).size()];
			children = (RelationType[]) RemoteUtils.relationTypeRemote
					.dbGetChildren(parent, LexiconManager.getInstance().getLexicons()).toArray(new RelationType[] {});
