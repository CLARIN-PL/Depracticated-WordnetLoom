...
public class ViwnLockerViewUI extends AbstractViewUI
	...
	/**
	 * This JPopupMenu is generated dynamically after clicking at the jl,
	 * its JMenuItems depends on a type of MarkedElement 
	 * */
	private JPopupMenu jpm;
	
	/** 
	 * Collection of object which listen for an event of synset selection change.
	 * */
	...
	/**
	 * Adds new marked element to locker/clipboard
	 * new element cannot be added to list if element with same node and ui is
	 * already on list
	 * element is added to list and dlm
	 * view is refreshed after this action automatically
	...
	/**
	 * Remove marked element from locker
	 * view is refreshed after this action automatically
...
//		}
	...
	/**
	 * MarkedElement class
	 * class used to create panels which are displayed as an elements of jl
	 * every type of data added to list should has its MerkedElementType
	 * and a constructor in MerkedElementClass, it could be done 
	 * by inheritance from that class... but who cares?
	 * */
	private class LockerElement extends JPanel {
		...
		/**
		 * need it to distinct a type of element, 
		 * to allow generate popups depending on type
		 * */
	 ...
	 * draw a panel as an element of JList
	 * */
	...
	/**
	 * LockerMouseAdapter
	 * handles mouse clicking at the locker
	 * */
		...
		public void mouseClicked(MouseEvent mE) {
					...
					jl.locationToIndex(mE.getPoint()) != -1)
			{
				if (SwingUtilities.isRightMouseButton(mE)) 
					// right mouse button to simply back to normal mode
	...
	/**
	 * @author amusial
	 * LockerElementRenderer for ViwnNode type
	 */
...
//				JLabel jl = new JLabel(vn.getLabel());
//				jl.setPreferredSize(new Dimension(100,15));
//				jp.add(jl, "br");
				...
				vv.getRenderContext().setVertexShapeTransformer(new Transformer<ViwnNode, Shape>() {public Shape transform(ViwnNode v) {return v.getShape();}});
